...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "The name of the Dataset (from the HuggingFace hub) to train on (could be"
            " your own, possibly private, dataset). It can also be a path pointing to a"
            " local copy of a dataset in your filesystem, or to a folder containing"
            " files that 🤗 Datasets can understand."
        ),
    )
    parser.add_argument(
        "--dataset_config_name",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "A folder containing the training data. Folder contents must follow the"
            " structure described in"
            " https://huggingface.co/docs/datasets/image_dataset#imagefolder. In"
            " particular, a `metadata.jsonl` file must exist to provide the captions"
    ...
    parser.add_argument(
        ...
        default=1,
        help=(
            "Run fine-tuning validation every X epochs. The validation process consists"
            " of running the prompt `args.validation_prompt` multiple times:"
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Whether to center crop the input images to the resolution. If not set, the"
    ...
    parser.add_argument(
        ...
        help=(
            ...
            " 'epsilon' or 'v_prediction' or leave `None`. If left to `None` the"
            " default prediction type of the scheduler:"
    ...
    parser.add_argument(
        ...
        default=500,
        help=(
            "Save a checkpoint of the training state every X updates. These checkpoints"
...
def main():
    ...
    if args.report_to == "wandb":
        if not is_wandb_available():
            raise ImportError(
                "Make sure to install wandb if you want to use it for logging during"
                " training."
            )
        import wandb
    ...
    # It's important to realize here how many attention weights will be added
    # and of which sizes
    ...
    # (see below) or specify a Dataset from the hub
    ...
    if args.image_column is None:
        ...
        if image_column not in column_names:
            raise ValueError(
                f"--image_column' value '{args.image_column}' needs to be one of:"
                f" {', '.join(column_names)}"
            )
    if args.caption_column is None:
        ...
        if caption_column not in column_names:
            raise ValueError(
                f"--caption_column' value '{args.caption_column}' needs to be one of:"
                f" {', '.join(column_names)}"
            )
    ...
    if args.resume_from_checkpoint:
        ...
        if path is None:
            accelerator.print(
                f"Checkpoint '{args.resume_from_checkpoint}' does not exist. Starting a"
                " new training run."
            )
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if args.snr_gamma is None:
                    ...
                    # Since we predict the noise instead of x_0,
                    # the original formulation is slightly changed.
                    ...
                    # We first calculate the original loss. Then we mean over the
                    # non-batch dimensions and
                    # rebalance the sample-wise losses with their
                    # respective loss weights.
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        if args.checkpoints_total_limit is not None:
                            ...
                            # before we save the new checkpoint, we need to have at
                            # _most_ `checkpoints_total_limit - 1` checkpoints
