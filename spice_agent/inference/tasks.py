...
from spice_agent.inference.types import (
    ...
    StableDiffusionXLControlNetPipelineConfig,
)
...
class StableDiffusionXLControlNetPipelineTask(
    ...
    def __init__(
        ...
        # Initialize configuration using options
        filtered_options = get_filtered_options(
            options, StableDiffusionXLControlNetPipelineConfig
        )
        if options["image"]:
            options["image"] = self.get_conditioned_image(options["image"])

        self.config = StableDiffusionXLControlNetPipelineConfig(**filtered_options)
